...
- 索引（Index - アルファベット順の単語＋ページ番号の羅列）
- 図表のキャプションすらない白紙・装飾のみのページ
- 出版社の宣伝・近刊リストなど本文と無関係なページ

以下は翻訳してください（"skip": false）:
- 目次（Table of Contents - 章タイトル・節タイトルをそのまま日本語化）
...
- 直訳より意訳寄り、ただし原意は保持
- 専門用語は適切に訳出（必要なら原語併記）
- **見出しのマークアップ**（通常の本文ページのみ）:
  - 章タイトル（"Chapter N: Title"、"Part N: Title" 等）→ 行頭に **# **
  - セクション見出し（章の中の節）→ 行頭に **## **
  - サブ見出し → 行頭に **### **
  - 通常の段落は何も付けない
- **目次（Table of Contents）ページの場合は例外**:
  - すべての項目を **通常の段落として翻訳**（# / ## / ### マーカーは一切使わない）
  - 元の階層（インデント・箇条書き）はテキストで再現してよい（例: "  ・第1章: ..."）
